...
namespace MapleShark
{
    public sealed class MapleAES
    {
        private readonly static byte[] sSecretKey = new byte[] {
            ...
            //(byte) 0x10, 0x00, 0x00, 0x00, (byte) 0x81, 0x00, 0x00, 0x00, (byte) 0x8A, 0x00, 0x00, 0x00,
            //(byte) 0x47, 0x00, 0x00, 0x00, (byte) 0x31, 0x00, 0x00, 0x00, (byte) 0x9E, 0x00, 0x00, 0x00,
            //(byte) 0x8C, 0x00, 0x00, 0x00, (byte) 0x9A, 0x00, 0x00, 0x00

            //131
         //       0x10, 0x00, 0x00, 0x00, 0x81, 0x00, 0x00, 0x00, 0x8A, 0x00, 0x00, 0x00,
         //(byte) 0x47, 0x00, 0x00, 0x00, 0x31, 0x00, 0x00, 0x00, 0x9E, 0x00, 0x00, 0x00,
         //       0x8C, 0x00, 0x00, 0x00, 0x9A, 0x00, 0x00, 0x00
         //132
             //0x5E , 0x00 , 0x00 , 0x00 , 0x9C , 0x00 , 0x00 , 0x00 , 0x9F ,
             //0x00 , 0x00 , 0x00 , 0x0C , 0x00 , 0x00 , 0x00 , 0x65 , 0x00 ,
             //0x00 , 0x00 , 0x89 , 0x00 , 0x00 , 0x00 , 0x7A , 0x00 , 0x00 ,
             //0x00 , 0x63 , 0x00 , 0x00 , 0x00
            0xDA, 0x00, 0x00, 0x00, 0x62, 0x00, 0x00, 0x00, 0x34,
            0x00, 0x00, 0x00, 0x3F, 0x00, 0x00, 0x00, 0x6D, 0x00,
            0x00, 0x00, 0x28, 0x00, 0x00, 0x00, 0x1D, 0x00, 0x00,
        ...
        public bool ConfirmHeader(byte[] pBuffer, int pStart)
        {
            ...
            return b;
            // return ((((pBuffer[0] ^ mIV[2]) & 0xFF) == ((mBuild >> 8) & 0xFF)) &&
            //(((pBuffer[1] ^ mIV[3]) & 0xFF) == (mBuild & 0xFF)));
        }
        ...
        public static byte[] multiplyBytes(byte[] inx, int count, int mul)
        {
        ...
        public byte[] TransformAES(byte[] data)
        {
            ...
            while (remaining > 0)
            {
                byte[] myIv = multiplyBytes(this.mIV, 4, 4);
                ...
                for (int x = start; x < (start + llength); x++)
                {
                    ...
                    {
                        byte[] newIv = Encrypt(myIv, sSecretKey); //AES加密
        ...
        public void TransformAES2(byte[] pData)
        {
